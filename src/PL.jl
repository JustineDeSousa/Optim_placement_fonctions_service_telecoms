...
conserv_s = @constraint(m, [k in 1:K], base_name="conserv_s",
	sum( x[s[k],j,k,1] for j in succ(s[k])) - sum( x[j, s[k],k,1] for j in prec(s[k]) ) - x[s[k],s[k],k,1] == 1)
conserv_t = @constraint(m, [k in 1:K], base_name="conserv_t",
	sum( x[t[k],j,k,C[k]] for j in succ(s[k])) - sum( x[j, t[k],k,C[k]] for j in prec(s[k]) ) - x[t[k],t[k],k,C[k]] == -1)
...
conserv = @constraint(m, [i=1:n, k in 1:K, c in 1:C[k]; i!=s[k] && i!=t[k]], base_name="Conserv_k_i",
	sum( x[i,j,k,c] for j in succ(i)) - sum( x[j, i,k,c] for j in prec(i) ) == 0)
...
function couche_of(k::Int, f::Int)
	index = findfirst(x->x==f, inst.orders[k,f])
	return index
end
capa_function = @constraint(m, [f in 1:F, i in 1:n], base_name="capa_funct_f_i",
	sum( x[i,i,k,inst.orders[k,couche_of(k,f)]] *inst.commod[k,3] for k in 1:K if couche_of(k,f) != nothing) <= inst.functions[f,1]*y[f,i])
...
order = @constraint(m, [i in 1:n, k in 1:K, c in 1:C[k]], base_name="orders_i", x[i,i,k,c] <= inst.orders[k,c])
ouverture_noeud = @constraint(m, [i in 1:n, k in 1:K, c in 1:C[k]], base_name="ouverture_noeud_i_k_c",
	x[i,i,k,c] <= u[i])
