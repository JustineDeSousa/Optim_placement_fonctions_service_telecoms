...
#################################################################################################
########### FONCTIONS CONSTRUCTION SOLUTION #####################################################
...
Retourne un objet Solution
"""
function place_functions(data::Data, paths::Vector{Vector{Int}}, max_time::Float64=10.0)
    startingTime = time()
    ...
    return Solution(paths, functions, data, 0, time() - startingTime)
end
""" 
Renvoie une solution initiale admissible en ignorant les contraintes d'ordre sur les fonctions
"""
function init_solution(data::Data, max_time::Float64=10.0)
...
########## CHECK SOLUTIONS ######################################################################
""" 
Renvoie true si ∀ client k, toutes les fonctions f commandées par k sont placées sur son chemin
"""
...
function costHeuristic(data::Data, solution::Solution; alpha::Int64=100, beta::Int64=100, gamma::Int64=100)
    ...
    _, orderFunc = areFunctionsOrdered(data, solution)
    ordCost = sum(orderFunc)
    return solution.cost + alpha * costConstV + beta * costExtraLatency + gamma * ordCost
end
"""
